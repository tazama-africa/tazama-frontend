      ...
      <template #title-data="{ row }">
        <div class="flex items-center lg:space-x-3 space-x-2 lg:w-full w-56">
          ...
          <div class="w-5/6 lg:w-auto">
            <span class="font-medium truncate lg:w-60 max-w-full whitespace-nowrap block text-white">{{ row.title
              }}</span>
            <span class="truncate max-w-xs text-xs lg:hidden block text-gray-200">{{ row.artist }}</span>
          </div>
        </div>
      </template>
