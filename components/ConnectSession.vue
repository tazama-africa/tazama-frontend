<template>
    ...
    <div v-else
        ...
        <div class="flex items-center px-2 justify-between">
            <div><div class="flex  pt-4 items-center gap-3 mb-2">
                    <svg xmlns="http://www.w3.org/2000/svg" class="w-6 h-6 text-blue-400"  viewBox="0 0 32 32">
            ...
            <nuxt-link :to="`/player/${fleetNo}`" class="flex justify-center items-center text-xs p-1 pl-2 gap-1 hover:underline hover:text-blue-400  border rounded-full  " >
                Join
                <span><svg xmlns="http://www.w3.org/2000/svg"  class="h-4"viewBox="0 0 24 24"><path fill="currentColor" d="m13.292 12l-4.6-4.6l.708-.708L14.708 12L9.4 17.308l-.708-.708z"/></svg></span>
            </nuxt-link>
        </div>
        <NowNext />
        ...
        <form @submit.prevent="handleSubmit"
            ...
            <input type="text" placeholder="Enter Code" v-model="playerNo" id="player" name="player_no"
                class="bg-transparent text-white text-sm flex-1 min-w-0 px-3 py-1 focus:outline-none placeholder-gray-400" />
            <button type="submit" id="submitBtn"
                class="bg-blue-600 hover:bg-sky-600 text-white text-xs font-medium px-4 py-2 rounded-lg transition-colors">
                Connect  
            </button>
        </form>
    </div>
