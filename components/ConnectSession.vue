        ...
        <form @submit.prevent="handleSubmit" class='flex gap-2 w-full'>
            ...
            <div class="relative w-full">
                <input v-model="playerNo" type="text" id="player" name="player_no"
                    class="block w-full p-4 py-2 ps-4 placeholder:text-white text-white text-sm lg:border-2 border border-blue-600 rounded-full bg-transparent focus:ring--500 focus:border--500 pl-5"
                    placeholder="Enter Code" required />
            </div>
            <button type="submit" id="submitBtn"
                class="bg-blue-600 w-48 text-white flex gap-2 items-center ps-3 pointer-cursor rounded-full px-4">
